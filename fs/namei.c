...
static inline int may_follow_link(struct nameidata *nd)
{
	...
	if (nd->inode && unlikely(nd->inode->i_state & INODE_STATE_SUS_PATH) && likely(susfs_is_current_proc_umounted())) {
		return -ENOENT;
	}
#endif
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (link->dentry->d_inode && unlikely(link->dentry->d_inode->i_state & INODE_STATE_SUS_PATH) && likely(susfs_is_current_proc_umounted())) {
		return -ENOENT;
	}
#endif
...
{
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (unlikely(inode->i_state & INODE_STATE_SUS_PATH) && likely(susfs_is_current_proc_umounted())) {
		return -ENOENT;
	}
#endif
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (!IS_ERR(dentry) && dentry->d_inode && unlikely(dentry->d_inode->i_state & INODE_STATE_SUS_PATH) && likely(susfs_is_current_proc_umounted())) {
		dput(dentry);
		return ERR_PTR(-ENOENT);
	}
#endif
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (!retval && path->dentry->d_inode && unlikely(path->dentry->d_inode->i_state & INODE_STATE_SUS_PATH) && likely(susfs_is_current_proc_umounted())) {
		putname(name);
		return -ENOENT;
	}
#endif
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (child->d_inode && unlikely(child->d_inode->i_state & INODE_STATE_SUS_PATH) && likely(susfs_is_current_proc_umounted())) {
		error = inode_permission2(mnt, dir, MAY_WRITE | MAY_EXEC);
		if (error) {
			return error;
		}
...
static int may_o_create(struct path *dir, struct dentry *dentry, umode_t mode)
{
	...
	if (dentry->d_inode && unlikely(dentry->d_inode->i_state & INODE_STATE_SUS_PATH) && likely(susfs_is_current_proc_umounted())) {
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	{
		if (unlikely(dentry->d_inode->i_state & INODE_STATE_SUS_PATH) && likely(susfs_is_current_proc_umounted())) {
			dput(dentry);
			return -ENOENT;
		}
		goto out_no_open;
